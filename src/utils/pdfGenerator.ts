...
/**
 * Calcula o bounding box de todos os elementos do canvas usando o DOM real
 * Considera as posições reais dos elementos no canvas (não transformadas)
 */
function calculateCanvasBoundsFromDOM(): { x: number; y: number; width: number; height: number } {
  // Encontra todos os elementos posicionados no canvas
  const allElements = document.querySelectorAll('[data-selecto-uid]')
  if (allElements.length === 0) {
    return { x: 0, y: 0, width: 800, height: 600 }
  }

  // Encontra o container do canvas que tem o transform
  const canvasContainer = document.querySelector('.canvas-background')?.parentElement?.querySelector('.relative.h-full.w-full') as HTMLElement
  if (!canvasContainer) {
    return { x: 0, y: 0, width: 800, height: 600 }
  }

  // Obtém o estado do zoom e pan
  const state = useLayoutStore.getState()
  const zoom = state.zoom || 1
  const panX = state.panX || 0
  const panY = state.panY || 0

  // Calcula bounds baseado nas posições dos elementos no store (coordenadas do canvas)
  // Não usa getBoundingClientRect porque ele retorna coordenadas transformadas
  const { rowGroups, rows, looseIds, trackersById, textElementIds, textElementsById } = state
  ...
  rowGroups.forEach((group) => {
    if (group.x !== undefined && group.x !== null) {
      minX = Math.min(minX, group.x)
      // Estima largura do grupo (será ajustado pelo DOM se necessário)
      maxX = Math.max(maxX, group.x + 800)
    }
    if (group.y !== undefined && group.y !== null) {
      minY = Math.min(minY, group.y)
      maxY = Math.max(maxY, group.y + 600)
    }
  })

  // Processa rows standalone
  rows
    .filter((r) => !r.groupId)
    .forEach((row) => {
      if (row.x !== undefined && row.x !== null) {
        minX = Math.min(minX, row.x)
        maxX = Math.max(maxX, row.x + 600)
      }
  ...
  // Processa loose trackers
  looseIds.forEach((id) => {
    const tracker = trackersById[id]
    if (tracker && tracker.x !== undefined && tracker.x !== null) {
      minX = Math.min(minX, tracker.x)
      maxX = Math.max(maxX, tracker.x + 120)
    }
    if (tracker && tracker.y !== undefined && tracker.y !== null) {
      minY = Math.min(minY, tracker.y)
      maxY = Math.max(maxY, tracker.y + 100)
    }
  })

  // Processa text elements
  textElementIds.forEach((id) => {
    const text = textElementsById[id]
    if (text && text.x !== undefined && text.x !== null) {
      minX = Math.min(minX, text.x)
      maxX = Math.max(maxX, text.x + 200)
    }
    if (text && text.y !== undefined && text.y !== null) {
      minY = Math.min(minY, text.y)
      maxY = Math.max(maxY, text.y + 50)
    }
  })

  // Se não encontrou elementos, tenta usar o DOM como fallback
  if (minX === Infinity) {
    ...
    allElements.forEach((el) => {
      ...
      const width = htmlEl.offsetWidth || 100
      const height = htmlEl.offsetHeight || 100
  ...
  // Adiciona padding generoso para garantir que nada seja cortado
  const padding = 100
  return {
